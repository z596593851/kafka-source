...
class SocketServer(val config: KafkaConfig, val metrics: Metrics, val time: Time) extends Logging with KafkaMetricsGroup {
  ...
  //processor线程的集合，包含了所有endpoint对应的processors线程
  ...
  def startup() {
    this.synchronized {
      ...
      endpoints.values.foreach { endpoint =>
        ...
        //遍历当前endpoint对应的processor集合
        for (i <- processorBeginIndex until processorEndIndex) {
          //创建processor对象
          processors(i) = newProcessor(i, connectionQuotas, protocol)
        }
        //创建acceptor，同时为processor创建对应线程
  ...
  //给requestChannel添加一个监听器，当handler线程向某个responseQueue中写入数据时，
  //会唤醒对应processor线程进行处理
...
private[kafka] class Acceptor(val endPoint: EndPoint,
  ...
  this.synchronized {
    //为acceptor的每个processor都创建对应的线程并启动
  ...
  override def run() {
    ...
    while (isRunning) {
      try {
        ...
        //给newConnections里的socketChannel注册OP_READ事件
        configureNewConnections()
        // register any new responses for writing
        processNewResponses()
        //底层调用的是KSelector.poll方法，读取请求发送响应
        poll()
        //KSelector.poll每次调用都会将读取的请求、发送成功的请求、以及断开的连接放入
        //completedReceives、completedSends、disconnected队列中等待处理，下面就处理相应的队列：
      ...
      try {
        curr.responseAction match {
          ...
          case RequestChannel.SendAction =>
            //调用KSelector.send方法，并将响应放入inflightResponses队列缓存
            sendResponse(curr)
