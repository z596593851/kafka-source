...
class Log(val dir: File,
  ...
  private def loadSegments() {
    ...
    //加载全部日志文件和索引文件。如果索引文件没有配对的日志文件，则删除索引文件；如果日志文件没有对应的索引文件，则重建索引文件。
    for(file <- dir.listFiles if file.isFile) {
      val filename = file.getName
      //处理索引文件
    ...
    try {
      ...
      lock synchronized {
        ...
        if (assignOffsets) {
          ...
          //获取nextOffsetMetadata记录的nextOffsetMetadata字段，从此值开始向后分配offset
          val offset = new LongRef(nextOffsetMetadata.messageOffset)
          //记录第一条消息的offset，并不受压缩消息的影响
  ...
  private def maybeRoll(messagesSize: Int, maxTimestampInMessages: Long): LogSegment = {
    ...
    val now = time.milliseconds
    val reachedRollMs = segment.timeWaitedForRoll(now, maxTimestampInMessages) > config.segmentMs - segment.rollJitterMs //当前activeSegment的寿命超过了配置的LogSegment最长存活时间
  ...
  def roll(): LogSegment = {
    ...
    lock synchronized {
      ...
      //segments的最后一个，也就是旧activeSegment
      segments.lastEntry() match {
        case null =>
        case entry => {
          val seg = entry.getValue
          seg.onBecomeInactiveSegment()
          //对索引文件和日志文件都进行截断，保证文件中只保存了有效字节，这对预分配的文件尤为重要
