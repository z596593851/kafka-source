...
public final class ConsumerCoordinator extends AbstractCoordinator {
    ...
    //metadata快照，用来检测topic是否发生了分区数量的变化
    ...
    protected void onJoinComplete(int generation,
        ...
        //needsFetchCommittedOffsets设置为true，允许从服务端获取最近一次提交的offset
    ...
    public void poll(long now) {
        ...
        if (subscriptions.partitionsAutoAssigned() && coordinatorUnknown()) {
            //Rebalance第一阶段：查找GroupCoordinator
            //这个阶段消费者会向集群中任意一个broker发送GroupCoordinatorRequest请求并处理响应
        ...
        if (needRejoin()) {
            ...
            if (subscriptions.hasPatternSubscription())
                //如果是AUTO_PATTERN模式，则检测metadata是否需要更新
                client.ensureFreshMetadata();

            //Rebalance第二阶段：消费者向GroupCoordinator发送JoinGroupRequest请求
            ensureActiveGroup();
            now = time.milliseconds();
        }
        pollHeartbeat(now);
    ...
    protected void onJoinPrepare(int generation, String memberId) {
        ...
        //如果是自动提交offset，则进行一次同步提交offset的操作
    ...
    public boolean needRejoin() {
        //检测是否是AUTO_TOPICS或AUTO_PATTERN模式
        if (!subscriptions.partitionsAutoAssigned())
            return false;
