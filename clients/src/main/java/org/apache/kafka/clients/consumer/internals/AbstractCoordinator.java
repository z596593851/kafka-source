...
public abstract class AbstractCoordinator implements Closeable {
    ...
    //服务端GroupCoordinator返回的年代信息，用来区分两次Rebabance操作，防止因为延迟收到上次的请求
    ...
    public synchronized void ensureCoordinatorReady() {
        while (coordinatorUnknown()) {
            //封装GroupCoordinatorRequest请求并放入client.send等待发送
    ...
    protected synchronized RequestFuture<Void> lookupCoordinator() {
        if (findCoordinatorFuture == null) {
            ...
            //找负载最低的节点
            //底层是找inFlightRequests中未确认请求最少的节点
    ...
    void joinGroupIfNeeded() {
        while (needRejoin() || rejoinIncomplete()) {
            //再次检测已经找到GroupCoordinator且已经建立了连接
            ensureCoordinatorReady();
            ...
            //创建JoinGroupRequest并放入unsent
            RequestFuture<ByteBuffer> future = initiateJoinGroup();
            //同步发送JoinGroupRequest
            client.poll(future);
            resetJoinGroupFuture();

            if (future.succeeded()) {
                needsJoinPrepare = true;
                //SyncGroupResponse中得到的分区分配结果最终由onJoinComplete处理
            ...
            if (error == Errors.NONE) {
                ...
                synchronized (AbstractCoordinator.this) {
                    if (state != MemberState.REBALANCING) {
                        ...
                        //消费者根据leaderId检测自己是不是leader
                        if (joinResponse.isLeader()) {
                            //此future是前面sendJoinGroupRequest返回的future
                            //这里使用chain()目的是当onJoinLeader的SyncGroupRequest处理完成后再通知此future
    ...
    private RequestFuture<ByteBuffer> onJoinLeader(JoinGroupResponse joinResponse) {
        try {
            ...
            //Rebalance第三阶段：完成分区分配之后，向GroupCoordinator发送SyncGroupRequest请求并处理SyncGroupResponse
    ...
    protected synchronized Node coordinator() {
        //与GroupCoordinator之间的网络连接是否正常
        if (coordinator != null && client.connectionFailed(coordinator)) {
            //将unsent中对应请求清空并设coordinator为null
            coordinatorDead();
            return null;
        }
